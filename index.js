const initialize_typewriter = () => {
  console.log('HERE')
  let typewriter_node = document.getElementById('typewriter');
  let addon = true;
  let i = 0;
  let type_arr = ['Artificial Intelligence', 'Acting Ill', 'Amazing Insights', 'Atrocious Ideas', 'An Ice-cream', 'Absolute Idiots', 'Accessing IKEA', 'Accommod8u Income', 'Adding Integrals', 'Astronomic Innovations', 'Adorable Iguanas', 'Alphabet Incompetence']
  let index = 0;
  let type_speed = 75;
  let delete_speed = 25;
  function typewrite() {
    if (addon) {
      if (i < 30) {
        typewriter_node.innerHTML += type_arr[index].charAt(i);
        i++;
      ...
      if (i > 0) {
        typewriter_node.innerHTML = typewriter_node.innerHTML.slice(0, -1);
        i--;
        setTimeout(typewrite, delete_speed);
      } else {
        addon = true;
        index++;
        if (index == type_arr.length) index = 0;
        setTimeout(typewrite, type_speed)
      }
    }
  }
